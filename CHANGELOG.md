...
1. 优化每日维护任务：仅重新校准自上次校准后发生过文件用量变化的账户，单次最多处理 200 个，避免按全体用户扫描。
2. 修复用量校准会重置当天上传累计的问题；校准现在只修正已用空间和文件数量，保留每日上传统计。
3. API Key 成功调用和已识别 Key 的拒绝调用改为按“Key + 日期”汇总，后台直接展示当天成功、流量、拒绝次数和最后拒绝原因。
4. 新增 API Key 日汇总保留期，默认 180 天，由既有计划任务自动清理过期统计，避免数据表持续增长。
5. 数据库升级至 `1011`；原有 API Key、配额、文件和分享保持不变。
