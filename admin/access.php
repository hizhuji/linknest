...
function loadKeys(){$.getJSON('ajax_access.php?act=list_keys',function(r){if(r.code!==0)return;var html='';$.each(r.rows,function(_,k){var state=k.revoked_at?'已撤销':(k.expires_at&&new Date(k.expires_at.replace(' ','T'))<new Date()?'已过期':'有效');html+='<tr><td>'+k.uid+' '+$('<div>').text(k.nickname||'').html()+'</td><td>'+$('<div>').text(k.name).html()+'<br><code>'+$('<div>').text(k.key_prefix).html()+'...</code></td><td>'+$('<div>').text(k.scopes).html()+'</td><td>'+($('<div>').text(k.expires_at||'长期').html())+'</td><td>'+k.today_requests+' 次 / '+mb(k.today_bytes)+'</td><td>'+state+'</td><td>'+(k.revoked_at?'':'<button class="btn btn-xs btn-danger" onclick="revokeKey('+k.id+')">撤销</button>')+'</td></tr>';});$('#keys').html(html||'<tr><td colspan="7" class="text-muted">暂无 API Key</td></tr>');});}
