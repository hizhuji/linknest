...
当前数据库版本为 `1011`。管理员更新程序文件后，只需登录并访问 `/install/update.php` 执行增量升级，不需要重新安装，也不会清空现有文件、账号或设置。

## V6.9 文件组织、配额与 API Key

- 登录用户可在“我的文件”为自己拥有的文件添加标签、收藏，并按标签、格式、大小或收藏状态筛选；后台文件管理也支持按标签、收藏和大小范围筛选。
- 后台“账户与接口”可查看、校准每个用户的用量，分别设置存储空间、文件数量和每日上传配额。默认仅统计；管理员启用强制开关后才拒绝超限上传。回收站中的文件继续占用空间，彻底删除后才释放。
- 新 API Key 绑定用户，支持上传、创建分享、读取元数据和删除自己文件四种独立权限，可设置来源 IP/CIDR、有效期、每日请求数和每日流量上限，随时撤销。完整 Key 只在创建时显示一次，数据库仅保存哈希。
- 用量校准只处理发生过变动的账户；API Key 成功与拒绝调用按天汇总并可设置保留期，避免后台维护随用户或请求总量线性膨胀。
...
上传 API 默认关闭。启用后可在后台设置来源域名白名单和旧版兼容令牌；推荐在“账户与接口”中为调用方创建受限 API Key，并通过 `X-Api-Key: lnk_...` 或 `Authorization: Bearer lnk_...` 发送。旧 `api_token` 仍兼容上传调用，但不具备新增的细粒度权限。
